...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	FIRStorageReference *reference = [[storage referenceForURL:FIREBASE_STORAGE] child:Filename(@"profile_picture", @"jpg")];
	...
	[task observeStatus:FIRStorageTaskStatusSuccess handler:^(FIRStorageTaskSnapshot *snapshot)
	{
		...
		[reference downloadURLWithCompletion:^(NSURL *URL, NSError *error)
		{
			if (error == nil)
			{
				...
				[self saveUserPicture:URL.absoluteString];
			}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	FIRStorage *storage = [FIRStorage storage];
	FIRStorageReference *reference = [[storage referenceForURL:FIREBASE_STORAGE] child:Filename(@"profile_thumbnail", @"jpg")];
	FIRStorageUploadTask *task = [reference putData:data metadata:nil completion:nil];
	//---------------------------------------------------------------------------------------------------------------------------------------------
	[task observeStatus:FIRStorageTaskStatusSuccess handler:^(FIRStorageTaskSnapshot *snapshot)
	{
		...
		[reference downloadURLWithCompletion:^(NSURL *URL, NSError *error)
		{
			if (error == nil)
			{
				[self saveUserThumbnail:URL.absoluteString];
			}
